...
const CreateNewQuotation = () => {
  const [formInstance] = Form.useForm(); // proxy
  ...
  const operator = Storage.getCachedDate("USER").username;
  ...
  const onFinish = (values: any) => {
    ...
    if (result.length === 0) {
      setTimeout(() => {
        ...
        // const currentClient = formInstance?.getFieldsValue(true);
        // const currentShippingInfo = formInstance?.getFieldValue("shipping");
        // const currentPaymentInfo = formInstance?.getFieldValue("price");
        // console.log(currentClient, currentShippingInfo, currentPaymentInfo);
        ...
        const payload = {
          ...
          clientVip: values.client.name,
          clientAddress: values.client.address,
          clientPostcode: values.client.postcode,
          clientMobile: values.client.mobile,
          checked: false,
          shippingAddress: values.shipping.address,
          shippingPostcode: values.shipping.postcode,
          shippingShippingFee: values.shipping.shippingFee,
          priceItemPrice: values.price.itemPrice,
          pricepcPrice: values.price.pcPrice,
          priceinstallPrice: values.price.installPrice,
          pricetotalAmount: values.price.totalAmount,
          pricedepositPayed: values.price.depositPayed,
