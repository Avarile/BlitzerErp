...
export const getOrdersByDate = async (dateRange: { startDate: number; finishDate: number }, setData: Function, setLoadingStatus: Function) => {
  setLoadingStatus(true);
  await Request.get(`${env.dbUri}/orders?createdAt_gte=${dateRange}&createdAt_lte=5`)
    .then((response) => {
      setData(response);
    })
    .finally(() => {
...
export const createNewClient = async (payload: { name: string; email: string; mobile: string; vip: boolean }) => {
  await Request.post(`${env.dbUri}/clients`, payload, {}, `User" ${payload.name}`);
};
